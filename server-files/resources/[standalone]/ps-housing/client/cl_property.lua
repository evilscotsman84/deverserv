...
function Property:EnterShell()
    ...
    RegisterCommand("ownermenu", function()
        if not self.inProperty then 
        QBCore.Functions.Notify('You are not in house', 'error')
        return
     end
        if self.owner then
...
function Property:GiveMenus()
    ...
    if self.owner or accessAndConfig then
        Modeler:OpenMenu(self.property_id, "ps-housing:client:openFurnitureMenu", { propertyId = self.property_id })
    end
end

function Property:OwnerMenu()
    if not self.inProperty then 
        QBCore.Functions.Notify('You are not in house', 'error')
        return
    end
    ...
    if self.owner then
        self:ManageAccessMenu()
        Modeler:OpenMenu(self.property_id, "ps-housing:client:openManagePropertyAccessMenu", { propertyId = self.property_id })
    end
end
...
RegisterNetEvent("openownermenu", function()
    ExecuteCommand("ownermenu")
end)

RegisterNetEvent("decoratehouse", function()
    ExecuteCommand("decorate")
end)
