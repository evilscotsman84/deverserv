...
$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (data.action == "AddCoord") {
            ...
            $("#BlipTop").val(50);
            $("#BlipLeft").val(50);
            $('.AddBlipForMap').append('<i style="top:'+50+'%; left:'+50+'%;" id="BlipSetting" class="fas fa-map-marker-alt IconClassStyle"></i>')
        }
    })
})
    ...
    if (type !== "lab") {
        ...
        if(SelectLocForSpawn == null){
            ...
            if(MainspawnType == "appartment"){
                ...
                $(".TextFoJSCode").html("Location: "+label)
            }
        ...
        if(Access.houses == true){
            if (myHouses != undefined) {
                $.each(myHouses, function(index, house){   
                    ...
                    var elem = '<li id="IconClassStyle" data-location="'+house.house+'" data-type="house" data-label="'+house.house+'"><p><span id="'+house.house+'">'+house.house.toUpperCase()+'</span></p></li>';
